...
namespace NumberMatrices
{
    ...
    class NumberMatrixGenerator
    {
        ...
        private void GetMatrixTypeB()
        {
            ...
            int number = 1;
            int currentRow = 0;
            int change = -1; // bottom to top

            for (int col = 0; col < this.size; col++)
            {
                change *= -1; // direction swap
                for (int row = 0; row < this.size; row++) // path traversal - constant
                {
                    this.matrix[currentRow, col] = number++;
                    currentRow += change; // path direction [in the size of the path]++
                }
                currentRow += -change; // trace 1 back - standing out of bound
            }
        }
